...
NS_LOG_COMPONENT_DEFINE ("PcapDumbell");

int main (int argc, char *argv[])
{

	LogComponentEnable("PacketSink", LOG_LEVEL_INFO);
	LogComponentEnable("PcapFileWrapper", LOG_LEVEL_INFO);
	LogComponentEnable("CsmaNetDevice", LOG_LEVEL_INFO);
	LogComponentEnable("UdpL4Protocol", LOG_LEVEL_INFO);
	
	PcapHelper pcapHelper;
	pcapHelper.EnableAsciiAll(std::cout);
	pcapHelper.EnablePcapAll("output.pcap", true);
